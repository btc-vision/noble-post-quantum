        ...
        if (C.keygen) {
            ...
            if (C.sign && C.verify) {
                ...
                if (C.info && C.info.type && C.info.type.startsWith('hash')) {
                    ...
                    CEG('verify: wrong ctx=', U8, ctx, (s) =>
                        C.verify(sig, msg, keys.publicKey, s),
                    );
                }
            }
            if (C.encapsulate && C.decapsulate) {
                ...
                CEG('decapsulate: wrong cipherText=', U8, msg, (s) =>
                    C.decapsulate(s, keys.secretKey),
                );
                CEG('decapsulate: wrong secretKey=', U8, msg, (s) =>
                    C.encapsulate(cipherText, msg),
                );
            }
            if (C.prehash) {
